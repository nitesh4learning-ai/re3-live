"use client";
// Interactive widgets for Academy MDX courses.
// Self-contained — no dependency on legacy Academy files.
...
const PROMPT_PRESETS={analyst:'You are a senior data analyst. Provide clear, data-driven insights with specific metrics. Always cite your reasoning.',teacher:'You are a patient and encouraging teacher. Explain concepts step-by-step using simple analogies. Check understanding frequently.',coder:'You are an expert software engineer. Write clean, well-commented code. Explain your design decisions and suggest best practices.'};
function PromptBuilder(){
  const[role,setRole]=useState('analyst');const[system,setSystem]=useState(PROMPT_PRESETS.analyst);const[user,setUser]=useState('');
  useEffect(()=>{setSystem(PROMPT_PRESETS[role])},[role]);
  return <div className="rounded-xl border p-4 mb-4" style={{borderColor:GIM.border,background:GIM.cardBg}}>
    ...
    <div className="flex gap-2 mb-3 flex-wrap">{Object.keys(PROMPT_PRESETS).map(r=><button key={r} onClick={()=>setRole(r)} className="px-3 py-1 rounded-full text-xs font-semibold capitalize" style={{background:role===r?GIM.primary:'white',color:role===r?'white':GIM.bodyText,border:`1px solid ${role===r?GIM.primary:GIM.border}`}}>{r}</button>)}</div>
...
function SchemaDesigner(){
  ...
  return <div className="rounded-xl border p-4 mb-4" style={{borderColor:GIM.border,background:GIM.cardBg}}>
    ...
    <div className="flex gap-2 mb-3 flex-wrap">{Object.keys(PROMPT_PRESETS).map(p=><button key={p} onClick={()=>setFields(presets[p])} className="px-3 py-1 rounded-full text-xs font-semibold capitalize" style={{background:GIM.primaryBadge,color:GIM.primary,border:`1px solid ${GIM.primary}22`}}>{p}</button>)}</div>
