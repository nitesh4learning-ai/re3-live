...
export function PillarTag({pillar,pillarData,size="sm"}){const p=PILLARS[pillar]||pillarData;if(!p)return null;return <span className={`inline-flex items-center gap-1 ${size==="sm"?"px-2 py-0.5 text-xs":"px-2.5 py-1 text-sm"} rounded-full font-semibold`} style={{background:p.lightBg||`${p.color}15`,color:p.color}}>{PILLARS[pillar]?<PillarIcon pillar={pillar} size={size==="sm"?11:13}/>:null}{p.label||pillar}</span>}
