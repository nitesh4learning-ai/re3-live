...
- Do NOT wrap the JSON in markdown code fences. Output the raw JSON directly.`;
    ...
    // Bound the spine's synthesis so its retries (gatewayCall does up to 3 attempts)
    // can't exhaust the serverless time budget; the rest of its budget was freed
    // by fast Gemini stages and skipping A2A.
    const synthesisTimeout = isContextSpine ? 70000 : 90000;

    const { text: synthesisText, usage: synthUsage } = await gatewayCall(
      "anthropic", // Synthesis always uses the best model
      synthesisSystem,
      synthesisUser,
      { maxTokens: synthesisMaxTokens, timeout: synthesisTimeout }
    );
