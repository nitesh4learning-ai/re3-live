...
import {
  ...
  CONTEXT_SPINE_MAX_AGENTS,
  buildContextSpine,
  buildSpineSynthesisSystem,
} from "./context-spine.js";
...
export async function runOrchestration(useCase, options = {}) {
  ...
  try {
    ...
    await loadRegistry();
    // context-comprehension expands into the fixed six-stage spine (deterministic,
    // no LLM); every other type uses the generic LLM decomposer.
    const isContextSpine = useCase.type === CONTEXT_COMPREHENSION_TYPE;
    const { tasks, complexity } = isContextSpine
      ? buildContextSpine(useCase)
      : await decompose(useCase, budget);
    ...
    // The spine has six stages — pin one persona per stage regardless of the
    // caller's maxAgents (still within the guardrail ceiling of 6).
    const effectiveMaxAgents = isContextSpine ? CONTEXT_SPINE_MAX_AGENTS : maxAgents;
    emit("phase.assemble.start", { agentPoolSize: 1000, maxAgents: effectiveMaxAgents }, null);

    const allAgents = await getAllAgents();

    // Select optimal team
    const team = selectTeam(allAgents, useCase.type, effectiveMaxAgents);
    ...
    // Skipped for the context spine: its six stages are independent and self-
    // contained, so an extra refinement round adds latency (a full second round
    // of LLM calls) without changing the per-stage answers — and the serverless
    // time budget is tight. Synthesis already sequences the stages together.
    let refinedResults;
    if (isContextSpine) {
      emit("a2a.skipped", { reason: "Context spine stages are independent" }, null);
      refinedResults = results;
    } else {
      refinedResults = await crossPollinate(
        results,
        taskMap,
        assignmentMap,
        board,
    ...
    // context-comprehension sequences the six stage analyses into a roadmap;
    // every other type produces the Arena blueprint + prototype deliverable.
    const synthesisSystem = isContextSpine
      ? buildSpineSynthesisSystem(useCase)
      : `You are the Synthesis Agent for the Re³ orchestration platform.
...
- Do NOT wrap the JSON in markdown code fences. Output the raw JSON directly.`;
    ...
    // Increase token budget for structured output. The spine keeps it leaner and
    // runs on Haiku (see below), so the roadmap generates fast.
    const synthesisMaxTokens = isContextSpine ? 3500 : 5000;

    const synthesisUser = isContextSpine
      ? `Here are the six stage analyses:\n\n${synthesisInput}\n\nNow weave them into the sequenced roadmap. Return ONLY valid JSON, no markdown wrapping.`
    ...
    // The spine runs entirely on Anthropic Haiku (fast + reliable, only needs the
    // Anthropic key). The 1,000-agent Arena synthesis stays on Sonnet ("standard").
    const { text: synthesisText, usage: synthUsage } = await gatewayCall(
      "anthropic",
      synthesisSystem,
      synthesisUser,
      { maxTokens: synthesisMaxTokens, timeout: 90000, tier: isContextSpine ? "light" : undefined }
    );
