...
import { CONTEXT_COMPREHENSION_TYPE } from "./context-spine.js";
...
export function buildDeliverable(board, synthesisText, results, budget) {
  ...
  // For the context spine, surface each stage's raw output text so the Context
  // page can render per-stage cards. Gated to this type so Arena deliverables
  // stay byte-for-byte unchanged.
  const isContextSpine = board.useCase?.type === CONTEXT_COMPREHENSION_TYPE;
  ...
  return {
    ...
    // context-comprehension only: the sequenced six-stage roadmap.
    roadmap: structured?.roadmap || null,
    team: board.team,
    taskResults: results.map((r) => ({
      ...
      error: r.error || null,
      ...(isContextSpine
        ? { output: readFromBoard(board, `${r.taskId}.output`) || null }
        : {}),
    })),
    metrics: {
