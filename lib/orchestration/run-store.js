...
export function saveRun({ deliverable, events, boardSnapshot, budget }) {
  ...
  saveRunToFirestore(record).catch((e) => {
    console.warn('saveRunToFirestore failed:', e.message || e);
  });
...
/**
 * One-time backfill: push every locally-stored run up to Firestore so the
 * shared Use Case Library is populated for the public.
 *
 * Runs that completed before the `orchestration_runs` Firestore rule existed
 * only ever reached localStorage (cloud writes were silently denied). This
 * re-uploads them. Idempotent (setDoc merge) and guarded to run once per page
 * load. Requires an authenticated user (writes need auth).
 */
let _backfilledThisSession = false;
export async function syncLocalRunsToCloud() {
  if (_backfilledThisSession) return { synced: 0, total: 0, alreadyRan: true };
  _backfilledThisSession = true;

  const mod = await getFirestoreModule();
  if (!mod) return { synced: 0, total: 0 };

  const runs = DB.get(STORE_KEY, []);
  let synced = 0;
  for (const record of runs) {
    try {
      await saveRunToFirestore(record);
      synced++;
    } catch {
      // Skip individual failures (e.g. oversized doc, transient error)
    }
  }
...
export async function listRunsCloud() {
  ...
  try {
    ...
    console.warn('listRunsCloud failed:', e.message || e);
    return localRuns;
  }
}
...
export async function getRunCloud(runId) {
  ...
  try {
    ...
    console.warn('getRunCloud failed:', e.message || e);
    return null;
  }
}
