...
Produce your output. Be thorough but concise.`;
    ...
    // Route to optimal model based on task complexity. A task may pin a specific
    // model/tier (e.g. the context spine forces Anthropic Haiku for speed +
    // reliability); otherwise fall back to complexity-based routing.
    const model = task._model || routeModel(task._complexity || 0.5, agent.model);
