...
// Stages route to a fast model (complexity in [0.3,0.6) → Gemini in routeModel),
// so the six run quickly in parallel without piling up on Anthropic and blowing
// the serverless time budget. The router falls back to Anthropic if Gemini is
// unavailable, and synthesis still uses the best model. The roadmap (synthesis)
// is where quality matters most; per-stage extraction is well within Gemini's range.
