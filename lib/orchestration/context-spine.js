...
// Stages are pinned to Anthropic Haiku (the "light" tier) via _model/_tier
// below — fast, reliable, and dependency-free (only the Anthropic key is needed).
// This avoids both slow Sonnet pile-ups and dead Gemini/Groq free-tier quotas
// that just 429 and fall back to Anthropic anyway. Synthesis still uses Sonnet,
// where the roadmap quality matters most. _complexity is kept for display/metrics
// but _model overrides routing.
const STAGE_COMPLEXITY = 0.5;
...
Answer the stage question for THIS specific system.`,
    ...
    _stage: stage.stage,
    _model: "anthropic", // pin to Anthropic Haiku (light) — fast + dependency-free
    _tier: "light",
  }));
