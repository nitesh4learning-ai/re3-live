// Firestore data layer for Re³
// Firestore is the SOURCE OF TRUTH; localStorage serves as offline cache.
// On load: fetch Firestore first, fall back to localStorage only when offline.
// On save: write to Firestore (debounced), update localStorage cache immediately.
// Conflict detection: updatedAt timestamps catch multi-tab overwrites.
// Collections: content, config (themes/agents/projects), articles, forge_sessions, users

import {
  db, doc, setDoc, getDoc, getDocs, collection, query,
  orderBy, limit, addDoc, updateDoc, serverTimestamp, where
} from './firebase';

// ==================== HELPERS ====================

const LS = {
  get: (key, fallback) => { try { const d = typeof window !== 'undefined' && localStorage.getItem(`re3_${key}`); return d ? JSON.parse(d) : fallback; } catch { return fallback; } },
  set: (key, val) => { try { typeof window !== 'undefined' && localStorage.setItem(`re3_${key}`, JSON.stringify(val)); } catch {} },
};

// Simple debounce for write operations
const debounceTimers = {};
const pendingWriteFns = {};
function debouncedWrite(key, fn, delay = 2000) {
  if (debounceTimers[key]) clearTimeout(debounceTimers[key]);
  pendingWriteFns[key] = fn;
  debounceTimers[key] = setTimeout(() => {
    delete pendingWriteFns[key];
    fn();
  }, delay);
}

// Flush all pending debounced writes immediately (call on beforeunload)
export function flushPendingWrites() {
  for (const [key, fn] of Object.entries(pendingWriteFns)) {
    if (debounceTimers[key]) clearTimeout(debounceTimers[key]);
    delete debounceTimers[key];
    delete pendingWriteFns[key];
    try { fn(); } catch (e) { console.warn('Flush write failed for', key, ':', e.message); }
  }
}

// ==================== CONFLICT DETECTION ====================
// Track last-known updatedAt per document to detect multi-tab overwrites.
// If Firestore doc has a newer updatedAt than what we last saw, another tab wrote first.

const knownTimestamps = {};

function toMillis(ts) {
  if (!ts) return 0;
  if (typeof ts.toMillis === 'function') return ts.toMillis();
  if (ts.seconds) return ts.seconds * 1000 + (ts.nanoseconds || 0) / 1e6;
  if (typeof ts === 'number') return ts;
  return 0;
}

function recordTimestamp(docPath, ts) {
  knownTimestamps[docPath] = toMillis(ts);
}

function hasConflict(docPath, remoteTs) {
  const known = knownTimestamps[docPath] || 0;
  const remote = toMillis(remoteTs);
  return remote > 0 && known > 0 && remote > known;
}

// Export for use in providers.js conflict resolution
export function getKnownTimestamp(docPath) {
  return knownTimestamps[docPath] || 0;
}

// ==================== USER PROFILE ====================

export async function saveUserProfile(user) {
  if (!user?.id) return;
  try {
    const ref = doc(db, 'users', user.id);
    await setDoc(ref, {
      ...user,
      updatedAt: serverTimestamp(),
    }, { merge: true });
  } catch (e) {
    console.warn('Firestore saveUserProfile failed:', e.message);
  }
}

export async function getUserProfile(userId) {
  if (!userId) return null;
  try {
    const snap = await getDoc(doc(db, 'users', userId));
    return snap.exists() ? snap.data() : null;
  } catch (e) {
    console.warn('Firestore getUserProfile failed:', e.message);
    return null;
  }
}

// ==================== CONTENT (Posts) ====================

export async function saveContent(contentArray) {
  if (!contentArray?.length) return;
  // Cache locally immediately (offline-first feel)
  LS.set('content_v5', contentArray);
  // Debounced Firestore write — Firestore is authoritative
  debouncedWrite('content', async () => {
    try {
      const batch = [];
      for (const post of contentArray.slice(0, 100)) {
        const ref = doc(db, 'content', post.id);
        batch.push(
          setDoc(ref, { ...post, updatedAt: serverTimestamp() }, { merge: true })
            .then(() => recordTimestamp(`content/${post.id}`, { seconds: Date.now() / 1000 }))
        );
      }
      const results = await Promise.allSettled(batch);
      const failed = results.filter(r => r.status === 'rejected');
      if (failed.length > 0) {
        console.warn(`Firestore saveContent: ${failed.length}/${results.length} writes failed`);
        failed.forEach(f => console.warn('  ->', f.reason?.message || f.reason));
      }
    } catch (e) {
      console.warn('Firestore saveContent failed:', e.message);
    }
  });
}

export async function loadContent(fallback = []) {
  const cached = LS.get('content_v5', null);
  try {
    const q = query(collection(db, 'content'), orderBy('updatedAt', 'desc'), limit(200));
    const snap = await getDocs(q);
    if (!snap.empty) {
      const posts = snap.docs.map(d => {
        const data = { id: d.id, ...d.data() };
        // Record timestamps for conflict detection
        recordTimestamp(`content/${d.id}`, data.updatedAt);
        delete data.updatedAt;
        return data;
      });
      LS.set('content_v5', posts);
      return { data: posts, source: 'firestore' };
    }
  } catch (e) {
    console.warn('Firestore loadContent failed, using offline cache:', e.message);
  }
  return { data: cached || fallback, source: 'cache' };
}

// ==================== THEMES ====================

export async function saveThemes(themesArray) {
  if (!themesArray) return;
  LS.set('themes', themesArray);
  debouncedWrite('themes', async () => {
    try {
      const ref = doc(db, 'config', 'themes');
      await setDoc(ref, { items: themesArray, updatedAt: serverTimestamp() });
      recordTimestamp('config/themes', { seconds: Date.now() / 1000 });
    } catch (e) {
      console.warn('Firestore saveThemes failed:', e.message);
    }
  });
}

export async function loadThemes(fallback = []) {
  const cached = LS.get('themes', null);
  try {
    const snap = await getDoc(doc(db, 'config', 'themes'));
    if (snap.exists()) {
      const data = snap.data();
      recordTimestamp('config/themes', data.updatedAt);
      const items = data.items || [];
      LS.set('themes', items);
      return { data: items, source: 'firestore' };
    }
  } catch (e) {
    console.warn('Firestore loadThemes failed:', e.message);
  }
  return { data: cached || fallback, source: 'cache' };
}

// ==================== ARTICLES ====================

export async function saveArticles(articlesArray) {
  if (!articlesArray) return;
  LS.set('articles_v1', articlesArray);
  debouncedWrite('articles', async () => {
    try {
      for (const article of articlesArray.slice(0, 50)) {
        const ref = doc(db, 'articles', article.id);
        await setDoc(ref, { ...article, updatedAt: serverTimestamp() }, { merge: true });
        recordTimestamp(`articles/${article.id}`, { seconds: Date.now() / 1000 });
      }
    } catch (e) {
      console.warn('Firestore saveArticles failed:', e.message);
    }
  });
}

export async function loadArticles(fallback = []) {
  const cached = LS.get('articles_v1', null);
  try {
    const q = query(collection(db, 'articles'), orderBy('updatedAt', 'desc'), limit(100));
    const snap = await getDocs(q);
    if (!snap.empty) {
      const articles = snap.docs.map(d => {
        const data = { id: d.id, ...d.data() };
        recordTimestamp(`articles/${d.id}`, data.updatedAt);
        delete data.updatedAt;
        return data;
      });
      LS.set('articles_v1', articles);
      return { data: articles, source: 'firestore' };
    }
  } catch (e) {
    console.warn('Firestore loadArticles failed:', e.message);
  }
  return { data: cached || fallback, source: 'cache' };
}

// ==================== AGENTS ====================

export async function saveAgents(agentsArray) {
  if (!agentsArray) return;
  LS.set('agents_v1', agentsArray);
  debouncedWrite('agents', async () => {
    try {
      const ref = doc(db, 'config', 'agents');
      await setDoc(ref, { items: agentsArray, updatedAt: serverTimestamp() });
      recordTimestamp('config/agents', { seconds: Date.now() / 1000 });
    } catch (e) {
      console.warn('Firestore saveAgents failed:', e.message);
    }
  });
}

export async function loadAgents(fallback = []) {
  const cached = LS.get('agents_v1', null);
  try {
    const snap = await getDoc(doc(db, 'config', 'agents'));
    if (snap.exists()) {
      const data = snap.data();
      recordTimestamp('config/agents', data.updatedAt);
      const items = data.items || [];
      LS.set('agents_v1', items);
      return { data: items, source: 'firestore' };
    }
  } catch (e) {
    console.warn('Firestore loadAgents failed:', e.message);
  }
  return { data: cached || fallback, source: 'cache' };
}

// ==================== PROJECTS ====================

export async function saveProjects(projectsArray) {
  if (!projectsArray) return;
  LS.set('projects_v1', projectsArray);
  debouncedWrite('projects', async () => {
    try {
      const ref = doc(db, 'config', 'projects');
      await setDoc(ref, { items: projectsArray, updatedAt: serverTimestamp() });
      recordTimestamp('config/projects', { seconds: Date.now() / 1000 });
    } catch (e) {
      console.warn('Firestore saveProjects failed:', e.message);
    }
  });
}

export async function loadProjects(fallback = []) {
  const cached = LS.get('projects_v1', null);
  try {
    const snap = await getDoc(doc(db, 'config', 'projects'));
    if (snap.exists()) {
      const data = snap.data();
      recordTimestamp('config/projects', data.updatedAt);
      const items = data.items || [];
      LS.set('projects_v1', items);
      return { data: items, source: 'firestore' };
    }
    // Doc doesn't exist yet — signal as firestore so caller can merge defaults
    return { data: [], source: 'firestore' };
  } catch (e) {
    console.warn('Firestore loadProjects failed:', e.message);
  }
  return { data: cached || fallback, source: 'cache' };
}

// ==================== EDITOR PICKS ====================

export async function saveEditorPicks(picksArray) {
  if (!picksArray) return;
  LS.set('editor_picks_v1', picksArray);
  debouncedWrite('editor_picks', async () => {
    try {
      const ref = doc(db, 'config', 'editor_picks');
      await setDoc(ref, { items: picksArray, updatedAt: serverTimestamp() });
      recordTimestamp('config/editor_picks', { seconds: Date.now() / 1000 });
    } catch (e) {
      console.warn('Firestore saveEditorPicks failed:', e.message);
    }
  });
}

export async function loadEditorPicks(fallback = []) {
  const cached = LS.get('editor_picks_v1', null);
  try {
    const snap = await getDoc(doc(db, 'config', 'editor_picks'));
    if (snap.exists()) {
      const data = snap.data();
      recordTimestamp('config/editor_picks', data.updatedAt);
      const items = data.items || [];
      LS.set('editor_picks_v1', items);
      return { data: items, source: 'firestore' };
    }
    return { data: [], source: 'firestore' };
  } catch (e) {
    console.warn('Firestore loadEditorPicks failed:', e.message);
  }
  return { data: cached || fallback, source: 'cache' };
}

// ==================== FORGE SESSIONS ====================

// Firestore does not support nested arrays (e.g. [[round1], [round2], [round3]]).
// These helpers convert nested arrays to/from object form at the Firestore boundary.
function flattenNestedArrays(obj) {
  if (Array.isArray(obj)) {
    const hasNestedArray = obj.some(item => Array.isArray(item));
    if (hasNestedArray) {
      const result = { _isArray: true, _length: obj.length };
      obj.forEach((item, i) => { result[String(i)] = flattenNestedArrays(item); });
      return result;
    }
    return obj.map(item => flattenNestedArrays(item));
  }
  if (obj && typeof obj === 'object' && !(obj instanceof Date)) {
    const result = {};
    for (const [key, val] of Object.entries(obj)) {
      result[key] = flattenNestedArrays(val);
    }
    return result;
  }
  return obj;
}

function restoreNestedArrays(obj) {
  if (obj && typeof obj === 'object' && obj._isArray) {
    const arr = [];
    for (let i = 0; i < obj._length; i++) {
      arr.push(restoreNestedArrays(obj[String(i)]));
    }
    return arr;
  }
  if (Array.isArray(obj)) {
    return obj.map(item => restoreNestedArrays(item));
  }
  if (obj && typeof obj === 'object' && !(obj instanceof Date)) {
    const result = {};
    for (const [key, val] of Object.entries(obj)) {
      result[key] = restoreNestedArrays(val);
    }
    return result;
  }
  return obj;
}

export async function saveForgeSessions(sessionsArray) {
  if (!sessionsArray) return;
  LS.set('forge_sessions_v1', sessionsArray);
  debouncedWrite('forge_sessions', async () => {
    try {
      for (const session of sessionsArray.slice(0, 50)) {
        const ref = doc(db, 'forge_sessions', session.id);
        const cleaned = flattenNestedArrays(session);
        await setDoc(ref, { ...cleaned, updatedAt: serverTimestamp() }, { merge: true });
        recordTimestamp(`forge_sessions/${session.id}`, { seconds: Date.now() / 1000 });
      }
    } catch (e) {
      console.warn('Firestore saveForgeSessions failed:', e.message);
    }
  });
}

export async function loadForgeSessions(fallback = []) {
  const cached = LS.get('forge_sessions_v1', null);
  try {
    const q = query(collection(db, 'forge_sessions'), orderBy('updatedAt', 'desc'), limit(50));
    const snap = await getDocs(q);
    if (!snap.empty) {
      const sessions = snap.docs.map(d => {
        const raw = { id: d.id, ...d.data() };
        recordTimestamp(`forge_sessions/${d.id}`, raw.updatedAt);
        delete raw.updatedAt;
        return restoreNestedArrays(raw);
      });
      LS.set('forge_sessions_v1', sessions);
      return { data: sessions, source: 'firestore' };
    }
  } catch (e) {
    console.warn('Firestore loadForgeSessions failed:', e.message);
  }
  return { data: cached || fallback, source: 'cache' };
}

// ==================== MIGRATION HELPER ====================
// One-time migration: push localStorage data to Firestore

export async function migrateLocalStorageToFirestore() {
  try {
    const content = LS.get('content_v5', null);
    const themes = LS.get('themes', null);
    const articles = LS.get('articles_v1', null);
    const agents = LS.get('agents_v1', null);
    const projects = LS.get('projects_v1', null);
    const forgeSessions = LS.get('forge_sessions_v1', null);

    const tasks = [];
    if (content?.length) tasks.push(saveContent(content));
    if (themes?.length) tasks.push(saveThemes(themes));
    if (articles?.length) tasks.push(saveArticles(articles));
    if (agents?.length) tasks.push(saveAgents(agents));
    if (projects?.length) tasks.push(saveProjects(projects));
    if (forgeSessions?.length) tasks.push(saveForgeSessions(forgeSessions));
    const editorPicks = LS.get('editor_picks_v1', null);
    if (editorPicks?.length) tasks.push(saveEditorPicks(editorPicks));

    await Promise.allSettled(tasks);

    // Mark migration as done
    LS.set('firestore_migrated', true);
    console.log('Re³ Firestore migration complete');
    return true;
  } catch (e) {
    console.warn('Firestore migration failed:', e.message);
    return false;
  }
}

export function needsMigration() {
  return !LS.get('firestore_migrated', false);
}
