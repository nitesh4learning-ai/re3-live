// Sliding window rate limiter using globalThis for serverless persistence.
// On Vercel, each serverless function process may handle multiple requests
// (warm invocations). Using globalThis ensures rate limit state persists
// across warm invocations within the same process.
//
// LIMITATION: Cold starts reset state. For true distributed rate limiting,
// use Redis (e.g. Upstash) — acceptable for alpha stage.

// Use globalThis to share state across all API routes in the same process
...
if (typeof globalThis !== "undefined" && typeof process !== "undefined" && process.env.NODE_ENV !== "test") {
  // Prevent duplicate intervals across module reloads (HMR)
