...
import { callLLM, callLLMWithRetry } from "./llm-router.js";
 ...
 */
const COST_PER_1K = {
  anthropic:          { input: 0.003, output: 0.015 },   // Sonnet (standard/heavy tier)
  "anthropic-light":  { input: 0.0008, output: 0.004 },  // Haiku (light tier)
...
export async function gatewayCall(model, systemPrompt, userMessage, opts = {}) {
  ...
  const text = await callLLMWithRetry(model, systemPrompt, userMessage, { maxTokens, timeout, retries: 2, tier: opts.tier });
  const durationMs = Date.now() - startMs;

  // Estimate output tokens from response
  const outputTokens = estimateTokens(text);
  const costKey = (model === "anthropic" && opts.tier === "light") ? "anthropic-light" : model;
  const rates = COST_PER_1K[costKey] || COST_PER_1K.anthropic;
  const estimatedCost =
